const ClassicalNoise = function(r?) {
  ...
  // To remove the need for index wrapping, double the
...
ClassicalNoise.prototype.noise = function(x, y, z) {
  ...
  const gi000 =
    this.perm[X + this.perm[Y + this.perm[Z]]] % 12
  const gi001 =
    this.perm[X + this.perm[Y + this.perm[Z + 1]]] % 12
  const gi010 =
    this.perm[X + this.perm[Y + 1 + this.perm[Z]]] % 12
  const gi011 =
    this.perm[X + this.perm[Y + 1 + this.perm[Z + 1]]] % 12
  const gi100 =
    this.perm[X + 1 + this.perm[Y + this.perm[Z]]] % 12
  const gi101 =
    this.perm[X + 1 + this.perm[Y + this.perm[Z + 1]]] % 12
  const gi110 =
    this.perm[X + 1 + this.perm[Y + 1 + this.perm[Z]]] % 12
  const gi111 =
    this.perm[X + 1 + this.perm[Y + 1 + this.perm[Z + 1]]] %
    12
